...
export async function archiveMoment(
  imageBlob: Blob,
  proof: any
): Promise<{ imageHash: string; proofHash: string; imageUrl: string; proofUrl: string }> {
  // 1. Upload the image first
  const imageResult = await uploadToLighthouse(imageBlob, `image_${proof.payload.hash}.jpg`);
  ...
  const proofBlob = new Blob([JSON.stringify(linkedProof, null, 2)], { type: "application/json" });
  const proofResult = await uploadToLighthouse(proofBlob, `proof_${proof.payload.hash}.json`);
